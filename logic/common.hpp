...
namespace common
{
	...
	inline std::unordered_map<int64_t, std::unordered_map<int64_t, Relation>> group_member_relations; // 群成员关系，外层 key 是 group_id，内层 key 是 user_id
	...
	inline void get_group_member_name(std::function<json(const std::string&, const json&)> api, const int64_t& group_id, const int64_t& user_id, std::string& name) {
