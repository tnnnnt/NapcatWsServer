...
class HandleMessage {
  ...
  private:
	...
	static void
	handle_upload_sex_image(const ApiFunc& api, const json& message_array, int64_t group_id, int64_t user_id);
	static bool
	...
	static void handle_no_permission(const ApiFunc& api, int64_t group_id, int64_t user_id);
};